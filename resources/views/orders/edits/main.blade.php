        ...
        <div class="card">
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="date" name="order_at" id="order_at" class="form-control" value="{{ date('Y-m-d',strtotime($data->order_at)) }}">
                </div>
                <div class="student">
                    <div class="form-group">
                        ...
                        <select name="student_id" id="student_id" class="form-control" required>
                            ...
                            @foreach ($students as $item)
                                <option value="{{ $item->id }}" {{ ($data->student_id == $item->id) ? 'selected' : '' }}>{{ $item->name }} | {{ $item->no_induk }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="amount">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="dibayar" id="dibayar" class="form-control" value="{{ number_format($data->terbayar,0,',','.') }}">
                    </div>
                    <div class="form-group">
                ...
                <div class="form-action">
                    <a href="{{ ($from === 'jurnal') ? route('jurnal.main') : route('order.main') }}" class="btn btn-default">Kembali</a>
                    <button type="button" class="btn btn-success" onclick="simpan(`{{ $data->invoice }}`)">Update</button>
                    <button type="button" class="btn btn-danger" onclick="deleteData(`{{ $data->invoice }}`)">Hapus</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    var strDataProducts = "{{ json_encode($dataProduct) }}"
    var dataProducts = JSON.parse(strDataProducts.replace(/&quot;/g, '"'))
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        if (result.success) {
                            ...
                            .then((result) => {
                                if(result.isConfirmed){
                                    window.location.href =  "{{ route('order.main') }}"
                                }
                            })
                        }
